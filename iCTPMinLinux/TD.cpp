...
void TD::ReqCommissionRate(const string &code, int requestID)
{
	...
	strcpy(req.BrokerID, m_mt->GetBrokerID().c_str()); 
	strcpy(req.InvestorID, m_mt->GetInvestorID().c_str()); 
	if((int)code.length() > 0)
	{
		strcpy(req.InstrumentID, code.c_str()); 
	}
...
void TD::ReqConfirmSettlementInfo(int requestID)
{
	...
	strcpy(settlementInfoConfirm.BrokerID, m_mt->GetBrokerID().c_str()); 
	strcpy(settlementInfoConfirm.InvestorID, m_mt->GetInvestorID().c_str()); 
...
void TD::ReqQryConfirmSettlementInfo(int requestID)
{
	...
	strcpy(qrySettlementInfoConfirm.BrokerID, m_mt->GetBrokerID().c_str()); 
	strcpy(qrySettlementInfoConfirm.InvestorID, m_mt->GetInvestorID().c_str()); 
...
void TD::ReqQryInvestorPosition(int requestID)
{
	...
	strcpy(qryInvestorPosition.BrokerID, m_mt->GetBrokerID().c_str()); 
	strcpy(qryInvestorPosition.InvestorID, m_mt->GetInvestorID().c_str()); 
...
void TD::ReqQryInvestorPositionCombineDetail(int requestID)
{
	...
	strcpy(qryInvestorPositionCombineDetail.BrokerID, m_mt->GetBrokerID().c_str()); 
	strcpy(qryInvestorPositionCombineDetail.InvestorID, m_mt->GetInvestorID().c_str()); 
...
void TD::ReqQryInvestorPositionDetail(int requestID)
{
	...
	strcpy(qryInvestorPositionDetail.BrokerID, m_mt->GetBrokerID().c_str()); 
	strcpy(qryInvestorPositionDetail.InvestorID, m_mt->GetInvestorID().c_str()); 
...
void TD::ReqQryOrderInfo(int requestID)
{
	...
	strcpy(qryOrder.BrokerID, m_mt->GetBrokerID().c_str()); 
	strcpy(qryOrder.InvestorID, m_mt->GetInvestorID().c_str()); 
...
void TD::ReqQryTradingAccount(int requestID)
{
	...
	strcpy(qryTradingAccount.BrokerID, m_mt->GetBrokerID().c_str()); 
	strcpy(qryTradingAccount.InvestorID, m_mt->GetInvestorID().c_str()); 
...
void TD::ReqQryTradeInfo(int requestID)
{
	...
	strcpy(qryTrade.BrokerID, m_mt->GetBrokerID().c_str()); 
	strcpy(qryTrade.InvestorID, m_mt->GetInvestorID().c_str()); 
...
int TD::ReqAuthenticate(int requestID)
{
	...
	memset(&req, 0, sizeof(req));
	strcpy(req.BrokerID, m_mt->GetBrokerID().c_str());
	strcpy(req.AppID, m_mt->GetAppID().c_str());
...
string TD::GetMaxOrderRef()
{
	m_maxOrderRef++;
	char strOrder[6];
	sprintf(strOrder, "%d", m_maxOrderRef);
...
{
	printf("CallBack:连接交易IP成功，开始看穿式验证\r\n");
...
{
	printf("CallBack:看穿式验证成功，开始登录交易\r\n");
...
void TD::OnRspError(CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
	if(pRspInfo)
	{
		if(m_mt)
		{
			...
			if(listener)
			{
				listener->OnLogCallBack(m_mt->GetTradingTime(), pRspInfo->ErrorMsg);
				printf("errorMSG:%s", pRspInfo->ErrorMsg);
			}
		}
	}
}
