...
void* CheckMT(void *lpParam)
{
	...
	if(mt->IsClearanced() && MT::IsClearanceTime())
	{
...
void* CheckLogined(void *lpParam)
{
	MT *mt = (MT*)lpParam;
	int i = 0;
	while(i < 5000)
	{
		...
		{
                     pthread_t checkMTThread;
                    pthread_create(&checkMTThread, 0, CheckMT, mt);
			return 0;
		}
		i += 10;
		::usleep(10 * 1000);
...
void* CheckTradingTime(void *lpParam)
{
	MT *mt = (MT*)lpParam;
	while(1)
	{
...
void* LoadMD(void *lpParam)
{
	printf("SEND:启动行情\r\n");
	MT *mt = (MT*)lpParam;
	mt->m_mdEvent = true;
	string mdConDir = MT::GetProgramDir() + "/data";
	if(!MT::IsDirectoryExist(mdConDir.c_str()))
	{
		MT::CreateDirectory(mdConDir.c_str());
	}
...
void* LoadTD(void *lpParam)
{
	printf("SEND:启动交易\r\n");
	MT *mt = (MT*)lpParam;
        mt->m_tdEvent = true;
	string tdConDir = MT::GetProgramDir() + "/data";
	if(!MT::IsDirectoryExist(tdConDir.c_str()))
	{
		MT::CreateDirectory(tdConDir.c_str());
	}
...
void MT::Start()
{
	if(IsTradingTime())
	{
		...
		m_connected = true;
		m_clearanced = true;
                pthread_t loadMDThread;
                pthread_create(&loadMDThread, 0, LoadMD, this);
                pthread_t loadTDThread;
                pthread_create(&loadTDThread, 0, LoadTD, this);
                pthread_t loginedThread;
                pthread_create(&loginedThread, 0, CheckLogined, this);
	}
	if(!m_checkingLogined)
	{
		m_checkingLogined = true;
                pthread_t checkThread;
                pthread_create(&checkThread, 0, CheckTradingTime, this);
                
                 pthread_t queryDataThread;
                pthread_create(&queryDataThread, 0, QueryData, this);
	}
}
